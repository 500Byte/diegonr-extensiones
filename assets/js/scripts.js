(function ($) {
   ...
   }; var egoSmoother = false; function smoothScroll() {
      let body = $('body'); if (body.hasClass('smooth-scroll')) {
         egoSmoother = true; $('#page').wrap('<div id="smooth-wrapper"></div>'); $('main').wrap('<div id="smooth-content"></div>'); if ((!siteHeader.hasClass('fixed')) && (!siteHeader.hasClass('sticky'))) {
   ...
   function egoDynamicList() {
      let list = $('.ego-dynamic-list'); list.each(function () {
         let list = $(this), item = list.find('ul').children('li'), image = item.find('.list-image'), animation = list.data('animation'), scrub = list.data('scrub'); animation ? list.find('a').addClass(animation) : ''; scrub ? list.find('a').data('scrub', true) : ''; item.each(function () {
            let $this = $(this); animation ? new egoTextAnimation($this.find('a')) : ''; $this.find('a').on('mouseenter', () => { list.addClass('hovered'); $this.addClass('current'); })
   ...
   function siteNavigation() {
      var resetNavigation = ''; function classicNavigation() {
         let nav = $('#site-navigation.classic'), overlay = $('.menu-overlay'), headerHeight = siteHeader.outerHeight(), subBg = nav.data('sub-background'); if (nav.hasClass('one-page')) {
            nav.children('ul').children('li').each(function () {
               ...
               $this.find('a').addClass('ego-scroll-button'); $this.find('a').attr('data-scroll-to', target);
            })
         }
         let hasChild = nav.find('.main-menu').children('.ego-has-children'); var resetHeader = gsap.delayedCall(.7, headerLayoutChange, [false, false, false, true]); resetHeader.kill(); setTimeout(function () {
            hasChild.each(function (i) {
               $(this).addClass('sub_id_' + i); let $this = $(this), classes = $this.attr('class'), arr = classes.split(' '), tltl = gsap.timeline({ id: 'sub_menu_tl_' + i }), x = 0; $this.find('.ego-sub-menu-wrap').addClass('sub_' + i).insertBefore(siteHeader)
      ...
      fullscreenNavigation(); function menuToggle(targetMenu) {
         var menuToggle = $('.menu-toggle'), overlay = $('.menu-overlay'), text = menuToggle.find('.toggle-text'), closeText = text.attr('data-close-text'), bgColor = $('#site-navigation.fullscreen_menu').css('backgroundColor'); text.wrapInner('<span data-close-text="' + closeText + '"></span>'); var headerReset = gsap.delayedCall(1, headerLayoutChange, [false, false, false, true]); headerReset.kill(); setTimeout(function () {
            let navTweens = gsap.getTweensOf(targetMenu.find('*').not('.main-menu *, .ego-button, .ego-button *')), i = 0, tltl = gsap.timeline({ id: 'fs_menu_tl' }); if (navTweens.length > 0) {
   ...
   }; function egoGeneralAnims(target) { target.each(function () { let $this = $(this); new egoGeneralAnimations($this) }) }
   function egoParallaxImages() {
   ...
   }; function egoImageAnims() { $('.has-anim-image').each(function () { let $this = $(this); new egoImageAnimation($this); }) }
   class egoTextAnimation {
   ...
   }; function egoTextAnims() {
      let hasAnim = $('.has-anim-text'), exclude = 'ul, li, a'; hasAnim.not(exclude).each(function () {
         let $this = $(this)
         let textAnim = new egoTextAnimation($this); matchMedia.add({ isMobile: "(max-width: 450px)" }, (context) => {
            if (textAnim.progress == 0) { textAnim.split.revert(); textAnim.tl.revert(); new egoTextAnimation($this); }
            return () => { }
   ...
   function egoListAnimations() { let list = $('ul'); list.each(function () { let $this = $(this), listItem = $this.find('li'), animation = $this.data('animation'); if (animation) { listItem.addClass(animation); listItem.addClass('no-revert'); listItem.each(function () { let $this = $(this); let anim = new egoTextAnimation($this); }) } }) }
   ...
   function egoProjectPages() {
      ...
      let nextProj = $('.ego-next-project'), link = nextProj.find('a'), nextTitle = nextProj.find('.next-project-title'), nextImage = nextProj.find('.next-project-image'); link.addClass('next-project-link')
   }
   function egoPortfolio() {
   ...
   function egoShopPage() {
      let products = $('.ego-products-grid'); products.each(function () {
   ...
   function showcaseFullscreenCarousel() {
      var showcaseFullscreenCarousel = $('.showcase-fullscreen-carousel'); showcaseFullscreenCarousel.each(function (i) {
         ...
         imagesWrap.find('.ego-video').each(function () { egoVideo($(this)) })
   ...
   function showcaseInfiniteGrid() {
      let fullscreenGrid = $('.showcase-infinite-grid'); fullscreenGrid.each(function (i) {
         ...
         requestAnimationFrame(raf); lenis.on('scroll', (e) => {
            var bgMarquee = gsap.getById('ego_marquee'), velocity = e.velocity / 15
   ...
   var switchedDark, switchedLight; function layoutSwitcher() {
      ...
      function applySystemThemePreference() {
         ...
         if (prefersDark && !body.hasClass('dark')) {
            switchDark.click();
         } else if (prefersLight && !body.hasClass('light')) {
            switchLight.click();
         }
      }

      let switcher = $('.ego-layout-switcher'), switchDark = switcher.find('.switch-dark'), switchLight = switcher.find('.switch-light'), follower = switcher.find('.active-follower'), body = $('body');
      ...
      window.matchMedia('(prefers-color-scheme: dark)').addListener(applySystemThemePreference);
      window.matchMedia('(prefers-color-scheme: light)').addListener(applySystemThemePreference);
      applySystemThemePreference();
   }
   function postsGrid() {
      var grid = $('.ego-posts-grid'); grid.each(function () {
   ...
   postsGrid()
   function egoCartButton() { var button = $('.ego-cart-button'); button.wrapInner('<span></span>') }
   function egoProductRatings() {
   ...
   function egoDynamicGrid() {
      const grid = $('.ego-dynamic-grid'); grid.each(function () {
         let grid = $(this), wrapper = grid.children('.grid--wrapper'), items = wrapper.children('.grid--item'), animation = grid.data('animation'); grid.hasClass('lightbox-gallery') ? grid.css('height', grid.outerHeight()) : ''; grid.hasClass('lightbox-gallery') ? egoLightboxGallery(wrapper, items) : ''; items.each(function (i) {
            i++
            ...
            if (animation != null) {
               ...
               grid.hasClass('image-grid') ? new egoImageAnimation($this) : new egoGeneralAnimations($this);
            }
         })
      })
   }
   function egoFilterableGrid() {
      let $scope = $('.ego-filterable-grid'), wrapper = $scope.find('.nfg--wrapper'), items = $scope.find('.nfg--item'), filtersWrapper = $scope.find('.nfg--filters--wrapper'), filters = $scope.find('.nfg--filters--wrapper li'); items.each(function (i) {
         i++
   ...
   function egoForms() {
      let form = $('.ego-form'); form.each(function () {
   ...
   function egoSingleProject() { $('.single-project').find('a').addClass('inner-project-link'); $('.single-project').find('.single-project-image').addClass('trans-media'); }
   function egoDynamicCarousel() {
      const carousel = $('.ego-dynamic-carousel'); carousel.each(function () {
         let carousel = $(this), wrapper = carousel.children('.carousel--wrapper'), items = wrapper.children('.carousel--item'), end, length = items.length, parentLeft, next = carousel.find('.carousel--next'), prev = carousel.find('.carousel--prev'), animation = carousel.data('animation'), customPin = carousel.data('pin'), speed = carousel.data('scroll-speed'), carouselDrag, wrapScroll, val; val = 0; end = wrapper.outerWidth() - items.last().outerWidth() - (items.last().outerWidth() / 2); parentLeft = carousel.offset().left, carousel.find('.carousel--total').html(length); items.each(function (i) {
            ...
            if (animation != null) {
               ...
               carousel.hasClass('image-carousel') ? new egoImageAnimation($this) : new egoGeneralAnimations($this);
            }
         })
   ...
   function egoIcons() { $('.ego-icon').each(function () { let $this = $(this); if ($this.hasClass('animate')) { ScrollTrigger.create({ trigger: $this, onEnter: () => { $this.addClass('is_inview') } }) } }) }
   function egoImageCompare() { let imageCompare = $('.image-compare'); imageCompare.each(function () { let $this = $(this), image1 = $this.find('.compare-image-1'), image2 = $this.find('.compare-image-2'); gsap.to(image1, { clipPath: 'inset(0% 100% 0% 0%)', scrollTrigger: { trigger: $this, scrub: 1, start: 'center center', end: 'bottom+=1000 top', pin: true } }) }) }
   function egoLightbox() {
   ...
   function egoVideo(vid) {
      var video = $(vid), mouseCursor = $('#mouseCursor'); if ((video.length) && (!video.hasClass('vid-ready'))) {
         video.each(function (i) {
            i++
            let $this = $(this), video = $this.find('.n-video'), playText = $this.data('play-text'), pauseText = $this.data('pause-text'), muteText = $this.data('mute-text'), unmuteText = $this.data('unmute-text'), nautoplay = $this.data('autoplay'), nmuted = $this.data('muted'), nloop = $this.data('loop'), play = $this.data('play'), controls; $this.data('controls') != false ? controls = $this.data('controls').split(" ") : ''; playText == null ? playText = 'Play' : ''; pauseText == null ? pauseText = 'Pause' : ''; muteText == null ? muteText = 'Mute' : ''; unmuteText == null ? unmuteText = 'Unmute' : ''; const egoVid = new Plyr(video, { controls: controls, clickToPlay: true, autopause: false, debug: false, fullscreen: false, storage: { enabled: false }, youtube: { modestbranding: 1, controls: 0, rel: 0, cc_load_policy: 0, iv_load_policy: 3, noCookie: true, frameborder: 0, }, vimeo: { autopause: false, controls: false, } }); if (!$this.hasClass('n-self')) {
               egoVid.autoplay = nautoplay
               egoVid.muted = nmuted
               egoVid.loop = nloop
            }
            egoVid.on('ready', (event) => {
               if ($this.find('video').attr('autoplay')) { egoVid.play() }
               $this.addClass('vid-ready'); $this.find('.plyr').addClass('ego_video_' + i); $this.addClass('ego-fake-running'); let vid = $('.ego_video_' + i); vid.find('.plyr__video-wrapper').addClass('ego-video-wrap'); vid.find('.plyr__controls').addClass('ego-controls-wrap'); vid.find('.plyr__control[data-plyr="play"]').append('<span class="hover-default ego-player-control ego-play">' + playText + '</span><span class="hover-default ego-player-control ego-pause">' + pauseText + '</span>')
               vid.find('.plyr__controls__item.plyr__volume button[data-plyr="mute"]').append('<span class="hover-default ego-player-control ego-mute">' + muteText + '</span><span class="hover-default ego-player-control ego-unmute">' + unmuteText + '</span>'); if ($this.hasClass('play-inner')) { $this.find('.ego-play-button').on('click', function () { if ($this.hasClass('lightbox-play')) { lightboxPlay(); } else { egoVid.muted = false; egoVid.restart(); egoVid.play(); $this.addClass('ego-running'); $this.removeClass('ego-fake-running'); } }) }
               if ($this.hasClass('play-cursor')) {
                  $this.on('click', function () {
                     if ($this.hasClass('lightbox-play')) {
                        ...
                        if ($this.hasClass('ego-fake-running')) {
                           $this.removeClass('cursor-icon'); $this.removeClass('cursor-text'); mouseCursor.removeClass('hover-size')
                           mouseCursor.removeClass('hover-text')
                           mouseCursor.removeClass('hover-icon')
                           egoVid.muted = false; egoVid.restart(); egoVid.play(); $this.addClass('ego-running'); $this.removeClass('ego-fake-running');
                        }
                     }
                  })
                  $this.on('mouseenter', function () {
                     if ($this.hasClass('ego-running')) {
               ...
               function lightboxPlay() {
                  ...
                  vid.find('.plyr__video-wrapper').addClass('ego-video-wrap'); vid.find('.plyr__controls').addClass('ego-controls-wrap'); vid.addClass('lightbox-open'); egoVid.muted = false; egoVid.restart(); egoVid.play(); $this.addClass('ego-running'); $this.removeClass('ego-fake-running'); if ($this.hasClass('ratio-adjusted')) { let $iframe = $('.nlv').find('.ego-video-wrap'); gsap.set($iframe, { clearProps: 'all' }); gsap.set($('.nlv').find('.plyr__video-embed__container'), { clearProps: 'all' }) }
                  $('.lightbox-close').on('click', function () {
                     gsap.to('.nlv', {
                        opacity: 0, duration: .3, onComplete: () => {
                           gsap.set(vid, { opacity: 1 })
                           vid.off('click'); vid.removeClass('nlv lightbox-open'); vid.find('.plyr__video-wrapper').removeClass('ego-video-wrap'); vid.find('.plyr__controls').removeClass('ego-controls-wrap'); egoVid.muted = true; vid.prependTo($this); $this.removeClass('ego-running'); $this.addClass('ego-fake-running');
                        }
                     })
                     if ($this.hasClass('ratio-adjusted')) {
                        let parent = $this.parent('div'); const targetHeight = parent.outerHeight(), parentWidth = parent.outerWidth(), parentHeight = parent.outerHeight(); $this.attr('ratio', egoVid.ratio); let rat = egoVid.ratio.split(':')[0] / egoVid.ratio.split(':')[1], parentRatio = parentWidth / parentHeight; if (parentRatio < rat) {
                           $this.addClass('ratio-adjusted'); function adjustIframeSize() {
                              const aspectRatio = rat; const $iframe = $this.find('.ego-video-wrap'); const targetWidth = targetHeight * aspectRatio; gsap.set($iframe, { width: targetWidth + 10, height: targetHeight + 10, x: -1 * (targetWidth - parentWidth) / 2 })
               ...
               if ($this.hasClass('play-on-hover')) {
                  egoVid.pause(); $this.on('mouseenter', () => { egoVid.play(); })
                  $this.on('mouseleave', () => { egoVid.pause(); })
               }
               if (egoVid.autoplay == true) { egoVid.play(); }
               ...
               setTimeout(function () {
                  ...
                  anim[0].parent.scrollTrigger !== null ? anim[0].parent.scrollTrigger.kill() : ''; egoVid.once('playing', (event) => { anim[0].parent.play(); });
               }, 2)
            }
            egoVid.once('playing', (event) => {
               if (egoVid.isEmbed) {
                  let parent = $this.parent('div'); const targetHeight = parent.outerHeight(), parentWidth = parent.outerWidth(), parentHeight = parent.outerHeight(); $this.attr('ratio', egoVid.ratio); let rat = egoVid.ratio.split(':')[0] / egoVid.ratio.split(':')[1], parentRatio = parentWidth / parentHeight; if (parentRatio < rat) {
                     $this.addClass('ratio-adjusted'); function adjustIframeSize() {
                        const aspectRatio = rat; const $iframe = $this.find('.ego-video-wrap'); const targetWidth = targetHeight * aspectRatio; gsap.set($iframe, { width: targetWidth + 10, height: targetHeight + 10, x: -1 * (targetWidth - parentWidth) / 2 })
   ...
   function egoHeading() { let heading = $('.ego-heading'); heading.each(function () { let heading = $(this), text = heading.children('*'), icon = heading.data('icon'); text.attr('data-icon', icon) }) }
   function egoCircleText() {
      let circularText = $(".ego-circular-text"); circularText.each(function (i) {
   ...
   function egoButtons() {
      var button = $('.ego-button'); button.each(function () {
         let button = $(this), $this = $(this).children('a'), icon = $this.find('.button-icon'), tl = gsap.timeline({ paused: true }), line, animation = $this.data('animation'), stagger = $this.data('stagger'), duration = $this.data('duration'); if (animation) {
            ...
            $this.addClass('detect-pov')
            new egoTextAnimation($this.find('.button-anim-holder'))
         }
         button.hasClass('underline') ? $this.append('<span class="button-line"></span>') : '';
      })
   }
   function egoMarquee(i) {
      var velocity; let marquee = $('.ego-marquee'); marquee.each(function () {
         ...
         infinityOnResize(); window.onresize = function () {
            $this.find('.ego-marquee:first-child').nextAll().remove()
            let infItem = $this.find('.infinity-item'), infWidht = infItem.outerWidth(), infLenght = window.innerWidth / infWidht; infinityOnResize();
         }
      })
   }
   function egoServices() {
      let egoServices = $('.ego-services'); egoServices.each(function () {
         let services = $(this), servicesItem = services.find('.ego-service'); servicesItem.append("<span class='services-item-bottom-line ego-service-line'></span>")
   ...
   function egoAccordion() {
      let egoAccordion = $('.ego-accordion'); egoAccordion.each(function () {
         let $this = $(this), acrdTitle = $this.find('.accordion-title'), acrdContent = $this.find('.accordion-content'), activeContent = $this.find('.active > .accordion-content'), icon = $this.data('toggle-icon'), animation = $this.data('animation'); if (animation) {
            ...
            new egoTextAnimation(acrdTitle.children('span'))
         }
   ...
   function egoClinetsCarousel() {
      let clientCarousel = $('.ego-clients-carousel'); clientCarousel.each(function () {
   ...
   function egoInfiniteCarousel() {
      let clientCarousel = $('.ego-infinite-carousel'); clientCarousel.each(function () {
   ...
   function egoTestimonials() {
      let testimonials = $('.ego-testimonials'); testimonials.each(function () {
   ...
   }; $('#noised-bg').length ? noise() : ''; function egoScrollButtons() {
      let button = $('.ego-scroll-button')
   ...
   function egoNumberCounter() {
      let numberCt = $('.ego-number-counter'); numberCt.each(function () {
   ...
   function egoTeamMember() {
      const member = $('.ego-team-member'); member.each(function () {
   ...
   function egoInfiniteTabs() {
      let egoInfiniteTab = $('.ego-infinite-tabs'); egoInfiniteTab.each(function () {
   ...
   function initPages() { $('.project-page').length ? egoProjectPages() : ''; egoPortfolio(); egoProductPage(); egoSinglePostPage() }
   function initPageElements() {
      egoIcons()
      egoHeading()
      egoCartButton()
      egoTeamMember()
      egoCircleText(); egoButtons(); egoMarquee()
      egoServices()
      egoNumberCounter(); egoAccordion(); egoInfiniteTabs(); egoClinetsCarousel(); egoInfiniteCarousel(); egoDynamicGrid(); egoFilterableGrid(); egoForms(); gridScroll()
      egoTestimonials()
      egoDynamicCarousel(); egoParallax(); egoLightbox()
      egoDynamicList(); detectPov()
      circleMaskBg(); egoSingleProject();
   }
   ...
   egoMouseCursor(true); smoothScroll(); $(window).on('load', function () {
      winLoaded = true; window.scrollTo(0, 0); egoVideo($('.ego-video')); egoGeneralAnims($('.has-anim')); egoTextAnims(); egoTextWrapper(); egoListAnimations(); egoImageAnims()
      egoParallaxImages(); initShowcases(); initPageElements(); egoSections(); initPages(); egoHeader(); siteNavigation(); egoScrollButtons(); layoutSwitcher(); ScrollTrigger.refresh(true); let loader = gsap.getById("innerTl"); if (loader != null) { setTimeout(function () { let instance = ScrollTrigger.getAll(), i = 0; for (i = 0; i < instance.length; i++) { instance[i].disable(); } }, 3) }
   })
      ...
      debug: true, timeout: 30000, transitions: [{
         ...
         name: 'projects-grid-transition', from: { custom: ({ trigger }) => { return trigger.classList && trigger.classList.contains('grid-project-link'); }, }, to: { namespace: ['project-boxed-image', 'project-fullscreen-image', 'project-half-image', 'project-creative', 'project-tall-image', 'project-image-gallery-horizontal', 'project-image-gallery-vertical', 'project-video'] }, leave(trigger) {
            return new Promise(function (resolve, reject) {
               ...
               if (media.find('.ego-video').length) { tl.to([otherProj, '.grid-controls', lastTar], { opacity: 0 }, 0) } else { tl.to([otherProj, media.parent('div').find('.masked'), '.grid-controls', lastTar], { opacity: 0 }, 0) }
               tl.to('hr', { width: 0 }, 0)
            })
         }, beforeEnter(data) {
            return new Promise(function (resolve, reject) {
         ...
         name: 'next-project-transition', from: { custom: ({ trigger }) => { return trigger.classList && trigger.classList.contains('next-project-link'); }, }, to: { namespace: ['project-boxed-image', 'project-fullscreen-image', 'project-half-image', 'project-creative', 'project-tall-image', 'project-image-gallery-horizontal', 'project-image-gallery-vertical', 'project-video'] }, leave() {
            return new Promise(function (resolve, reject) {
               let tl = gsap.timeline({ onComplete: () => { resolve(); } })
               let media = $('.next-project-image'), parent = $('.ego-next-project'), targets = ['div.next-project-caption , .next-project-title, .next-project-metas'], bg = parent.css('backgroundColor'), target; headerLayoutChange(false, bg, false, true)
               tl.to(window, {
   ...
   barba.hooks.before((data) => { startLoading(); }); barba.hooks.enter((data) => {
      ...
      egoSmoother == true ? $('body').addClass('smooth-scroll') : ''; $('body').attr('style', bodyStyles); $('.site-header').attr('class', headerClasses); if (!$('.site-header').hasClass('blend')) {
      ...
      $('html').removeClass('loading')
      enableScroll(); egoTextAnims(); egoTextWrapper(); egoGeneralAnims($('main .has-anim')); egoListAnimations(); egoImageAnims()
      egoParallaxImages(); initShowcases(); initPageElements(); egoSections(); initPages(); egoVideo($('#primary').find('.ego-video')); ScrollTrigger.refresh(true); egoMouseCursor(false); setTimeout(function () {
         ...
         siteHeader.removeClass('sticked'); gsap.set([siteHeader, stickyTargets, siteHeader.children('div'), siteHeader.find('.site-logo'), siteHeader.find('.sticky-logo')], { clearProps: 'all' }); egoHeader(); ScrollTrigger.refresh(true)
      }, 200)
   });
}(jQuery));